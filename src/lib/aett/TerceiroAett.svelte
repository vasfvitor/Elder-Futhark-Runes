...
{#key idx}
  <div
    ...
    class="atropos h-[435px] w-[300px] max-h-screen max-w-prose {runa.nome} bg-black p-4 runic-card-atropos mx-auto cursor-default select-none"
  >
    ...
    <div class=" atropos-scale">
      ...
      <div
        class:active={over}
        class="card-frame atropos-rotate transition-all bg-anim p-2"
      >
        <!-- inner container (required) bg-repeat bg-contain bg-center -->
        <div
          class="atropos-inner rounded card-bg relative text-[10rem] text-center"
        >
          <div
...
<style>
  .card-frame {
    ...
    background-color: var(--runic-bg);
    background-image: url("/cards/pattern1.svg");
  }
  .card-bg {
    background-repeat: repeat;
    background-size: contain;
    background-color: var(--runic-bg);
    background-image: url("/cards/pattern2.svg");
  }
